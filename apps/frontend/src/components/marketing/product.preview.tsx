...
export function ProductPreview() {
  return (
    <div className={styles.productPreview} aria-label={`${brand.name} post composer and approval workflow preview`}>
      <div className={styles.previewChrome}><div><i /><i /><i /></div><span>{brand.name} / composer</span><b>PREVIEW</b></div>
      <div className={styles.previewBody}>
        ...
        <section className={styles.composerPane}>
          <div className={styles.composerHeading}><div><span>NEW POST</span><h3>Create a post</h3></div><button type="button" disabled>Save draft</button></div>
          ...
          <div className={styles.composerFooter}><span>Tomorrow · 09:30</span><button type="button" disabled>Request approval</button></div>
        </section>
